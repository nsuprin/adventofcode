...
INPUT;
  ...
  private static $sMode = self::MODE1;
  ...
  public static function parseLine($sLine) {
    list($sRule, $sPassword)  = explode(': ', $sLine);
    list($sRange, $sChar)     = explode(' ', $sRule);
    list($iMin, $iMax)        = explode('-', $sRange);
    return self::checkPassword($sPassword, $sChar, $iMin, $iMax);
  }

  public static function checkPassword($sPassword, $sChar, $iMin, $iMax) {
    switch (self::$sMode) {
      case self::MODE1:
        $iCount = substr_count($sPassword, $sChar);
        $bReturn = ($iMin <= $iCount && $iCount <= $iMax);
        break;
      case self::MODE2:
        $sA = substr($sPassword, $iMin - 1, 1);
        $sB = substr($sPassword, $iMax - 1, 1);
        $bReturn  = ($sA == $sChar xor $sB == $sChar);
        break;
    }
    return $bReturn;
  }
