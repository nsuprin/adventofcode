...
INPUT;
  ...
  private static $aTmpParse = [];

  private static $sSearchedColor  = 'shiny gold';
...
TEST;
  ...
  public static function parse($aInput) {
    $aBags  = self::parseBags($aInput);

    self::search($aBags);
   return sizeof(self::$aTmpParse);
  }
   ...
   * @return int|mixed
   */
  public static function search($aBags) {
    $iReturn = 0;
    foreach ($aBags as $sBag => $aLinkedBag) {
      if ($sBag == self::$sSearchedColor) {
        continue;
      }
      $iReturn += self::crawl($aBags, $sBag);
    }
    return $iReturn;
  }
   ...
   * @return int|mixed
   */
  public static function crawl($aBags, $sBag) {
    $iReturn  = 0;
    if (array_key_exists(self::$sSearchedColor, $aBags[$sBag])) {
      $iReturn++;
    } else {
      foreach ($aBags[$sBag] as $sChildBag => $nb) {
        //echo "$sBag>$sChildBag ($nb).\n";
        $iReturn  += self::crawl($aBags, $sChildBag);
      }
    }
    if ($iReturn > 0) {
      self::$aTmpParse[$sBag] = $sBag;
    }
    return $iReturn;
  }
   ...
   * @return array
   */
  public static function parseBags($aInput) {
    $aBags  = [];
    foreach ($aInput as $sLine) {
      list($sBagName, $sContain) = explode(' bags contain ', $sLine);
      $aBags[$sBagName]  = self::parseBag($sContain);
    }
    return $aBags;
  }
   ...
   * @return array
   */
  public static function parseBag($sContains) {
    $aContains = explode(', ', $sContains);
    $aContain = [];
    foreach ($aContains as $sContain) {
      if (preg_match('/(\d)\s([\w\s]+)(?<!bag)\sbag/', $sContain, $aMatches)) {
        list(,$number,$bag) = $aMatches;
        $aContain[$bag] = $number;
      }
    }
    return $aContain;
  ...
  public static function parse2($aInput) {
    $aBags  = self::parseBags($aInput);
    $iCount = self::count($aBags, self::$sSearchedColor);
    var_dump($iCount);
    return $iCount;
  }

  public static function count($aBags, $sParentBag) {
    $iCount = 0;
    foreach ($aBags[$sParentBag] as $sBag => $iNb) {
      $iCount += $iNb;
      var_dump([$sBag, $iNb]);
      $iCount  += ($iNb * self::count($aBags, $sBag));
    }
    return $iCount;
  }
