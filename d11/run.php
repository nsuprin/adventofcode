...
INPUT;
  ...
  public static function parse($aInput)
  {
      ...
      do {
          $process  = true;
          $aRoom = self::fillRoom($aRoom);
          $sMd5 = md5(serialize($aRoom));
          if (!isset($aProcessed[$sMd5])) {
            $aProcessed[$sMd5]  = 0;
          }
          $aProcessed[$sMd5]++;
  ...
  public static function render($aRoom) {
  ...
  public static function buildRoom($aInput) {
      $aRoom    = [];
      foreach ($aInput as $iLine => $sLine) {
          $aRoom[$iLine]    = str_split($sLine);
      }
      return $aRoom;
  }

  public static function fillRoom($aRoom)  {
      $aNewRoom = $aRoom;
      self::$iOccupied  = 0;
      foreach ($aRoom as $x => $aRow) {
  ...
  public static function getNewStatus($aRoom, $srcX, $srcY) {
      ...
      if (self::FLOOR != $sCurrentSeat) {
          ...
          for ($x = ($srcX - 1) ; $x <= $srcX + 1 ; $x++) {
              if (!isset($aRoom[$x])) {
                  continue;
              }
              for ($y = ($srcY - 1) ; $y <= $srcY + 1 ; $y++) {
                  if (!isset($aRoom[$x][$y])) {
                      continue;
                  }
                  if ($x == $srcX && $y == $srcY) {
