...
INPUT;
  ...
  public static function run() {
    $aInput = explode("\n", self::$sInput);
    var_dump(array_sum(self::parse($aInput)));
  }

  public static function test() {
    $aInput = explode("\n", self::$sTest);
    if (11 == array_sum(self::parse($aInput))) {
      var_dump('OK');
    } else {
      var_dump('KO');
    }
  ...
  public static function parse2($aInput) {
    ...
    $aGroups = self::getGroups2($aInput);
    $aMarks = [];
    foreach ($aGroups as $aGroup) {
      if (1 == sizeof($aGroup)) {
        $aMarks[] = sizeof($aGroup[0]);
      } else {
        //var_dump($aGroup);
        $aMarks[] =sizeof(call_user_func_array('array_intersect_key', $aGroup));
      }
    }
    //}
    //var_dump(array_sum($aMarks));
    return (array_sum($aMarks));
  }


  public static function getGroups2($aInput) {
    $aPassports = [];
    $aPassport  = [];
    ...
    foreach ($aInput as $iLine => $sLine) {
      ...
      if (0 == strlen($sLine) || $iLine >= $iLastLine) {
        ...
        foreach ($aPassportTmp as $sPassportLine) {
          ...
          $aPassport[]  = array_flip(str_split($sPassportLine));
        }
        //var_dump($aPassport);
   ...
   * @return array
   */
  public static function getGroups($aInput) {
    $aPassports = [];
    $aPassport  = [];
    $iLastLine  = sizeof($aInput) - 1 ;
    foreach ($aInput as $iLine => $sLine) {
      ...
      if (0 == strlen($sLine) || $iLine >= $iLastLine) {
        ...
        foreach ($aPassportTmp as $sPassportLine) {
          ...
          $aPassport  = array_merge($aPassport, array_flip(str_split($sPassportLine)));
        }
        //var_dump($aPassport);
        $aPassports[] = sizeof($aPassport);
        $aPassport  = [];
      }
    }
    return $aPassports;
