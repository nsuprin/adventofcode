...
INPUT;
  ...
  public static function test2() {
    ...
    var_dump(true == self::passportIsValid2(['byr' => 2002]));
    var_dump(false == self::passportIsValid2(['byr' => 2003]));

    var_dump(true == self::passportIsValid2(['hgt' => '60in']));
    var_dump(true == self::passportIsValid2(['hgt' => '190cm']));
    var_dump(false == self::passportIsValid2(['hgt' => '190in']));
    var_dump(false == self::passportIsValid2(['hgt' => '190']));


    var_dump(true == self::passportIsValid2(['hcl' => '#123abc']));
    var_dump(false == self::passportIsValid2(['hcl' => '#123abz']));
    var_dump(false == self::passportIsValid2(['hcl' => '123abc']));


    var_dump(true == self::passportIsValid2(['ecl' => 'brn']));
    var_dump(false == self::passportIsValid2(['ecl' => 'wat']));

    var_dump(true == self::passportIsValid2(['pid' => '000000001']));
    var_dump(false == self::passportIsValid2(['pid' => '0123456789']));

    $aInvalides  = <<< INVALIDES
eyr:1972 cid:100
...
VALIDES;
    ...
    var_dump(0 == self::parse2(explode("\n", $aInvalides)));
    var_dump(4 == self::parse2(explode("\n", $aValides)));
  ...
  public static function parse2($aInput) {
    $iValids  = 0;
    $aPassports = self::getPassports($aInput);
    var_dump(sizeof($aPassports));
    foreach ($aPassports as $aPassport) {
      if (self::passportIsValid($aPassport)) {
        if (self::passportIsValid2($aPassport)) {
          $iValids++;
        }
      }
    }
  ...
  public static function getPassports($aInput) {
    $aPassports = [];
    $aPassport  = [];
    $iLastLine  = sizeof($aInput) - 1 ;
    foreach ($aInput as $iLine => $sLine) {
      $aPassport[]  = $sLine;
      if (0 == strlen($sLine) || $iLine >= $iLastLine) {
        $aPassportTmp = explode(' ', implode(' ', $aPassport));
        $aPassport  = [];
        foreach ($aPassportTmp as $sPassportLine) {
          if (empty($sPassportLine)) {
            continue;
          }
          list($key, $value)  = explode(':', $sPassportLine);
          $aPassport[$key]  = $value;
        }
        $aPassports[] = $aPassport;
        $aPassport  = [];
      }
    }
    return $aPassports;
  ...
  public static function passportIsValid($aPassport) {
    $aPassportKeys = array_keys($aPassport);
    $aDiffs = array_diff(self::$aRequired, $aPassportKeys);

    $bValid = false;
    if (sizeof($aDiffs) ==  1) {
      if ('cid' == implode('', $aDiffs)) {
        $bValid = true;
      }
    } elseif (0 == sizeof($aDiffs)) {
      $bValid = true;
    }
    // var_dump([implode(' ', $aPassportKeys), implode(' ', $aDiffs), $bValid]);
    return $bValid;
  }

  public static function passportIsValid2($aPassport) {
    $bValid  = true;
    foreach ($aPassport as $key => $value) {
      switch ($key) {
        case 'byr':
          $bValid = (1920 <= $value  && $value <= 2002);
          break;
        case 'iyr':
          $bValid = (2010 <= $value  && $value <= 2020);
          break;
        case 'eyr':
          $bValid = (2020 <= $value  && $value <= 2030);
          break;
        case 'hgt':
          $bValid  = self::checkHgt($value);
          break;
        case 'hcl' :
          $bValid = preg_match('/^#[\da-f]{6}$/', $value);
          break;
        case 'ecl' :
          $bValid = preg_match('/^(amb|blu|brn|gry|grn|hzl|oth)$/', $value);
          break;
        case 'pid' :
          $bValid = preg_match('/^\d{9}$/', $value);
          break;
      }
      if (!$bValid) {
        break;
  ...
  public static function checkHgt($value) {
    $bValid = false;
    if (preg_match("/^(\d{2,3})(cm|in)$/", $value, $aMatches)) {
      list(, $hgt, $unit)  = $aMatches;
      switch ($unit) {
        case 'cm' :
          $bValid = (150 <= $hgt  && $hgt <= 193);
          break;
        case 'in' :
          $bValid = (59 <= $hgt  && $hgt <= 76);
          break;
      }
    }
    return $bValid;
